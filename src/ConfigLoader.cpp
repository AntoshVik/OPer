...
std::vector<Algorithm> ConfigLoader::loadFromDirectory(const std::string& dirPath) {
    ...
    if (!fs::exists(dirPath) || !fs::is_directory(dirPath)) {
        std::cerr << "OPer: Config directory does not exist: " << dirPath << std::endl;
        return algorithms;
    }
    ...
    for (const auto& entry : fs::directory_iterator(dirPath)) {
        ...
        if (!f.is_open()) {
            std::cerr << "OPer: Cannot open " << entry.path() << std::endl;
            continue;
        }
        try {
            json j;
            ...
            std::cout << "OPer: Loaded algorithm: " << j.at("pattern").get<std::string>() << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "OPer: Error parsing " << entry.path() << ": " << e.what() << std::endl;
        }
    }
    return algorithms;
}